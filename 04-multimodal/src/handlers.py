...
from openai import APIError, InternalServerError, NotFoundError, audio
...
@router.message(lambda message: message.voice or (message.audio and message.audio.mime_type.startswith("audio/")))
async def handle_audio(message: Message):
    chat_id = message.chat.id
    
    logger.info(f"Audio received from {chat_id}")
    ...
    try:
        ...
        # Преобразуем аудио в текст (транскрипция)
        transcribed_text = await transcribe_audio(file_buffer)  # функция, которая возвращает текст
        ...
        # Детальное логирование ответа LLM
        logger.info(f"LLM response for audio from {chat_id}: answer='{response.answer[:200]}...', transactions_count={len(response.transactions)}")
        if response.transactions:
            logger.info(f"Extracted {len(response.transactions)} transactions from audio for {chat_id}: {[t.model_dump() for t in response.transactions]}")
        else:
            logger.warning(f"No transactions extracted from audio for {chat_id}")
        ...
        # Добавляем аудио в историю как текстовое описание (для контекста)
        chat_conversations[chat_id].append(
            {"role": "user", "content": f"[Голосовое сообщение: распознано как текст: '{transcribed_text[:100]}...']"}
        )
        ...
        logger.error(f"Error processing audio from {chat_id}: {e}", exc_info=True)
        await message.answer(
...
model = WhisperModel("base")  # или "small", "medium", "large"

async def transcribe_audio(audio) -> str:
    """
    Транскрибирует аудио через faster-whisper (кроссплатформенно, Windows OK)
    :param audio_bytes: байты аудио
    :param fmt: формат аудио ("ogg", "mp3", "wav")
    :return: распознанный текст
    """

    segments, info = await asyncio.to_thread(model.transcribe, audio)
